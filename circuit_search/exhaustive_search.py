...
class ExhaustiveSearch(LayerCompilation):

    def __init__(self, num_layers, **kwargs):

        """
        Exhaustive search over all possible subcircuits of a given length.
    ...
    def save_results(self, folder):
        ...
        np.savetxt(os.path.join(folder, "best_fidelities.txt"), 1 - np.asarray(self.best_error_list))
        np.savetxt(os.path.join(folder, "best_circuit_length.txt"), np.asarray(self.best_circuit_size_list))
        np.savetxt(os.path.join(folder, "best_cricuits.txt"), np.asarray(self.best_circuit_list))
        np.savetxt(os.path.join(folder, "best_angle_params.txt"), np.asarray(self.best_angles).flatten())

    def run_search(self):
        """
        Performs an exhaustive search over all possible subcircuits of a given length.
        It extends the LayerCompilation class. It is a brute force search, so it is not very scalable (factorial complexity).
        But it guarantees to find the optimal circuit for small numbers of layers.
        :return:
        """
        ...
        for l_idx in range(start, self.num_layers + 1):
            ...
            for m in range(1, (l_idx + 1) * self.num_qubits + 3 * (l_idx + 1) - 1):
                ...
                for sub_circ in all_subcirc_combinations:
                    circuit = general_circuit.copy()
                    circuit = np.delete(circuit, sub_circ)
                    print(circuit)
                    # print(circuit)
